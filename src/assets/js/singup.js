(function () {
  ...
  const btnsListDest = [
    ...btnsList.querySelectorAll(`.singup-hero__btn-section`),
  ];
  const images = document.querySelectorAll(".slider .src");
  const maxImagesIndex = images.length;
  const SLIDERTIME = 4000;
  // eslint-disable-next-line no-unused-vars
  const autoSlider = window.matchMedia(`(min-width: 1200px)`).matches
    ? setInterval(moveSlider, SLIDERTIME, "autorun")
    : undefined;
  let index = 0;
  ...
  function verifySubmission(event) {
    ...
    inputs.forEach((element) => {
      ...
      if (element.type === "text" && value.length <= 2) {
        ...
        const elementErrorBox =
          element.parentElement.parentElement.nextElementSibling;
        const nodeError = createNodeError(
          "To continue you must have to accept the Terms and Conditions"
        );
        elementErrorBox.appendChild(nodeError);
        element.setAttribute("aria-invalid", true);
        flagInvalid = true;
  ...
  function togglePassword() {
    let input = passwordBtn
      .closest(".form-password-box")
      .querySelector("#form__password");
